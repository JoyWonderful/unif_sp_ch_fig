...
class Generator_figfont:
    ...
    Each function in this class starts with `ch_` corresponding a font generator.\n\n\
    ...
    def ch_filling(self, ch_fill="\u2588\u2588", ch_blank="  ") -> Figfont: # ch_fill = "██" 
        ...
        while True:
            try:
                i = next(iter_bin_dic)
                ret_fig.font_dic.update({i: list(map(lambda line:line.replace("1",ch_fill).replace("0",ch_blank), self.bin_dic[i]))})
            except StopIteration:
                break
